...
def create_Qr_Code(request):
    ...
    if request.method == "POST":
        ...
        qrgen2(
            ...
            f"{base_url}login",
            random_password,
            f"media/userqrcodes/{random_user}.png",
        )

        user = User(
            user_name=random_user,
            user_password=random_password,
            user_info_qrcode=f"media/userqrcodes/{random_user}",
        )
        user.save()
        ...
        qrgen(
            ...
            f"{base_url}brandurl",
            brand_name.decode(),
            brand_product_name.decode(),
            f"media/qrcodes/{brand_logo_formatted}",
            brand_logo.decode(),
        )
        ...
        brand = Brand(
            ...
            brand_logo=brand_logo_raw,
            brand_qr_code_picture=f"media/qrcodes/{brand_logo_formatted}",
            brand_qr_code_url=brand_qr_code_url,
        )
        brand.save()

        # Our json Data.
        field = {
            "Brand": brand_name.decode("utf-8"),
            "Product": brand_product_name.decode("utf-8"),
            "BrandLogo": f"media/qrcodes/{brand_logo_formatted}",
            "BrandQRUrl": brand_qr_code_url,
        }
        ...
        with Image.open(
            f"media/brandlogos/{brand_logo_raw.name.replace(' ','_')}"
        ) as image:
            image.thumbnail((256, 256))
            image.save(
                f"media/brandlogos/thumbnails/{brand_logo_formatted}", quality=100
            )
